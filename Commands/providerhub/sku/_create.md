...
- sku create
    ```bash
        providerhub sku create --provider-namespace "{providerNamespace}" --resource-type "{resourceType}" --sku "{skuName}" --sku-settings "[{name:freeSku,tier:Tier1,kind:Standard},{name:premiumSku,tier:Tier2,kind:Premium,costs:[{meter-id:xxx}]}]"
    ```
